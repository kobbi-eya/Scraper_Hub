...
# ── Concurrence ───────────────────────────────────────────────────────────────
# Toutes les routes des services (youtube, twitter, snapchat, reddit, linkedin)
# sont des `def` classiques (pas `async def`) car elles font des appels réseau
# bloquants (requests, yt-dlp, etc.). FastAPI les exécute donc automatiquement
# dans le threadpool d'anyio, ce qui permet à plusieurs clients d'être servis
# en parallèle sans geler la boucle asyncio. La limite par défaut (40 threads)
# est trop faible pour un scraper à forte latence réseau -> on l'augmente.
@asynccontextmanager
